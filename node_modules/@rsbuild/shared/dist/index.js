...
var getSharedPkgCompiledPath = (packageName) => path2.join(__dirname, "../compiled", packageName);
...
function getFilename2(config, type, isProd2) {
...
async function getBrowserslist(path3) {
  const env2 = getNodeEnv();
  const cacheKey = path3 + env2;
  if (browsersListCache.has(cacheKey)) {
    return browsersListCache.get(cacheKey);
  }
  const result = browserslist.loadConfig({ path: path3, env: env2 });
  if (result) {
...
async function getBrowserslistWithDefault(path3, config, target) {
  ...
  if (target === "web" || target === "web-worker") {
    ...
    const browserslistrc = await getBrowserslist(path3);
    if (browserslistrc) {
...
var urlJoin = (base, path3) => {
  const fullUrl = new URL(base);
  fullUrl.pathname = posix.join(fullUrl.pathname, path3);
