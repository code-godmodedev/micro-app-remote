...
${color.yellow(tips.join("\n"))}`;
    ...
    ensureAbsolutePath = (base, filePath) => path2.isAbsolute(filePath) ? filePath : path2.resolve(base, filePath);
    isFileSync = (filePath) => {
      try {
...
var OVERRIDE_PATH, isOverridePath, merge, mergeRsbuildConfig;
...
function getDefaultEntry(root) {
  const files = [
    ...
    "mjs",
    "cjs"
  ].map((ext) => join2(root, `src/index.${ext}`));
...
var init_config = __esm({
  "src/config.ts"() {
    ...
    getDefaultDevConfig = () => ({
      hmr: true,
      liveReload: true,
      assetPrefix: DEFAULT_ASSET_PREFIX2,
      startUrl: false,
      client: {
        overlay: true
      }
    ...
    withDefaultConfig = async (rootPath, config) => {
      ...
      if (!merged.source.tsconfigPath) {
        const tsconfigPath = join2(rootPath, TS_CONFIG_FILE);
    ...
    resolveConfigPath = (root, customConfig) => {
      if (customConfig) {
        const customConfigPath = isAbsolute(customConfig) ? customConfig : join2(root, customConfig);
...
`);
      ...
      for (const file of CONFIG_FILES) {
        const configFile = join2(root, file);
...
function loadEnv({
  ...
  const filePaths = filenames.map((filename) => join3(cwd, filename)).filter(isFileSync);
  const parsed = {};
  for (const envPath of filePaths) {
    Object.assign(parsed, parse(fs2.readFileSync(envPath)));
  }
  expand({ parsed });
  const publicVars = {};
...
// src/initHooks.ts
import { isFunction as isFunction2 } from "@rsbuild/shared";
...
async function createContextByConfig(options, bundlerType, config = {}) {
  ...
  const cachePath = join4(rootPath, "node_modules", ".cache");
...
var init_pluginHelper = __esm({
  "src/pluginHelper.ts"() {
    ...
    getHTMLPlugin = () => {
      if (!htmlPlugin) {
        htmlPlugin = __require("html-webpack-plugin");
      }
      return htmlPlugin;
    };
...
async function inspectConfig({
  ...
  if (!isAbsolute3(outputPath)) {
    outputPath = join6(context.rootPath, outputPath);
  }
    ...
    getHtmlFallbackMiddleware = ({ htmlFallback, distPath, callback, outputFileSystem }) => {
      return async (req, res, next) => {
        ...
        if (pathname.endsWith("/")) {
          ...
          const filePath = path4.join(distPath, pathname, "index.html");
          ...
          const filePath = path4.join(distPath, `${pathname}.html`);
          if (await isFileExists2(filePath)) {
            return rewrite(newUrl);
          }
        }
        if (htmlFallback === "index") {
          if (await isFileExists2(path4.join(distPath, "index.html"))) {
    ...
    applyDefaultMiddlewares = async ({
      ...
      compileMiddlewareAPI && middlewares.push(
        getHtmlFallbackMiddleware({
          distPath: isAbsolute4(distPath) ? distPath : join7(pwd, distPath),
...
`
    ...
    getDevConfig = ({
      ...
      const devConfig = config.dev ? deepmerge(defaultDevConfig, config.dev) : defaultDevConfig;
      return devConfig;
    };
    getIpv4Interfaces = () => {
          ...
          (chain, { env, isProd: isProd6, target, bundler, CHAIN_ID: CHAIN_ID3 }) => {
            ...
            if (env === "development") {
              chain.output.devtoolModuleFilenameTemplate(
                (info) => path5.resolve(info.absoluteResourcePath).replace(/\\/g, "/")
              );
            }
...
async function applyCSSRule({
  ...
  if (target === "web") {
    ...
    rule.use(CHAIN_ID3.USE.IGNORE_CSS).loader(path6.join(LOADER_PATH, "ignoreCssLoader.cjs")).end();
  }
...
var enableNativeCss, isUseCssExtract, getCSSModulesLocalIdentName, normalizeCssLoaderOptions, userPostcssrcCache, applyAutoprefixer, getPostcssLoaderOptions, getCSSLoaderOptions, pluginCss;
var init_css = __esm({
  "src/plugins/css.ts"() {
    "use strict";
    init_esm();
        ...
        api.modifyBundlerChain(
          async (chain, { CHAIN_ID: CHAIN_ID3, target, isProd: isProd6, isServer, isServiceWorker }) => {
            ...
            if (config.output.copy && api.context.bundlerType === "rspack") {
              ...
              chain.plugin(CHAIN_ID3.PLUGIN.COPY).use(rspack5.CopyRspackPlugin, [options]);
            }
...
async function printFileSizes(config, stats, rootPath) {
  ...
  const formatAsset = (asset, distPath, distFolder) => {
    ...
    return {
      size,
      folder: path7.join(distFolder, path7.dirname(fileName)),
      name: path7.basename(fileName),
      gzippedSize,
  ...
  const assets = multiStats.map((stats2) => {
    ...
    const distFolder = path7.relative(rootPath, distPath);
  ...
  for (const asset of assets) {
    ...
    const fileNameLength = (folder + path7.sep + name).length;
...
var filterAsset, getAssetColor, calcFileSize, coloringAssetName, pluginFileSize;
        ...
        api.modifyBundlerChain((chain, { isProd: isProd6, target }) => {
          ...
          const createAssetRule = (assetType, exts, emit2) => {
            ...
            chainStaticAssetRule({
              ...
              rule,
              maxSize,
              filename: path8.posix.join(distDir, filename),
              assetType
            });
          };
...
import {
  ...
  deepmerge as deepmerge3,
  isObject as isObject2
} from "@rsbuild/shared";
import { rspack as rspack6 } from "@rspack/core";
var getSwcMinimizerOptions, parseMinifyOptions, pluginMinimize;
  ...
  "src/plugins/minimize.ts"() {
    ...
    getSwcMinimizerOptions = (config) => {
      ...
      if (removeConsole === true) {
        options.compress = {
          ...isObject2(options.compress) ? options.compress : {},
          drop_console: true
        };
      ...
      if (jsOptions) {
        return deepmerge3(options, jsOptions);
      }
      return options;
    };
    ...
    pluginMinimize = () => ({
      ...
      setup(api) {
        ...
        api.modifyBundlerChain(async (chain, { isProd: isProd6 }) => {
          ...
          const { SwcJsMinimizerRspackPlugin, SwcCssMinimizerRspackPlugin } = rspack6;
          const { minifyJs, minifyCss } = parseMinifyOptions(config);
          if (minifyJs) {
            chain.optimization.minimizer(CHAIN_ID2.MINIMIZER.JS).use(SwcJsMinimizerRspackPlugin, [getSwcMinimizerOptions(config)]).end();
          }
          if (minifyCss) {
            chain.optimization.minimizer(CHAIN_ID2.MINIMIZER.CSS).use(SwcCssMinimizerRspackPlugin, []).end();
          }
        });
      }
    });
...
var init_HtmlAppIconPlugin = __esm({
  "src/rspack/HtmlAppIconPlugin.ts"() {
    ...
    HtmlAppIconPlugin = class {
      ...
      apply(compiler) {
        if (!fs4.existsSync(this.iconPath)) {
          throw new Error(
        ...
        compiler.hooks.thisCompilation.tap(
          ...
          (compilation) => {
            compilation.hooks.processAssets.tap(
              ...
              () => {
                const source = fs4.readFileSync(this.iconPath);
...
async function getTemplate(entryName, config, rootPath) {
  ...
  const absolutePath = isAbsolute6(templatePath) ? templatePath : path9.resolve(rootPath, templatePath);
...
var existTemplatePath, getTagConfig, pluginHtml;
var init_html = __esm({
  "src/plugins/html.ts"() {
    "use strict";
    init_esm();
...
async function getDefaultSwcConfig(config, rootPath, target) {
  return {
    ...
    isModule: "unknown",
    env: {
      targets: await getBrowserslistWithDefault3(rootPath, config, target)
    }
  };
}
...
var builtinSwcLoaderName, pluginSwc;
var init_swc = __esm({
  "src/plugins/swc.ts"() {
    "use strict";
    init_esm();
...
var init_InlineChunkHtmlPlugin = __esm({
  "src/rspack/InlineChunkHtmlPlugin.ts"() {
    ...
    InlineChunkHtmlPlugin = class {
      ...
      apply(compiler) {
        compiler.hooks.compilation.tap(this.name, (compilation) => {
          const publicPath = getPublicPathFromCompiler3(compiler);
    ...
    pluginInlineChunk = () => ({
      ...
      setup(api) {
        api.modifyBundlerChain(async (chain, { target, CHAIN_ID: CHAIN_ID3, isDev: isDev3 }) => {
          const config = api.getNormalizedConfig();
          if (isHtmlDisabled2(config, target) || isDev3) {
            return;
          }
          ...
          const styleTests = [];
          if (inlineScripts) {
            scriptTests.push(inlineScripts === true ? JS_REGEX2 : inlineScripts);
          }
          if (inlineStyles) {
...
function determineAsValue({
  ...
  const extension = path12.extname(url2.pathname).slice(1);
...
import {
  isHtmlDisabled as isHtmlDisabled3
} from "@rsbuild/shared";
    ...
    pluginResourceHints = () => ({
      ...
      setup(api) {
        ...
        api.modifyBundlerChain(async (chain, { CHAIN_ID: CHAIN_ID3, target }) => {
          ...
          if (isHtmlDisabled3(config, target)) {
            return;
          }
...
import {
  DEFAULT_ASSET_PREFIX as DEFAULT_ASSET_PREFIX4
} from "@rsbuild/shared";
  ...
  return {
    ...
    setup(api) {
      ...
      api.modifyBundlerChain(async (chain, { CHAIN_ID: CHAIN_ID3, target }) => {
        ...
        chain.plugin(CHAIN_ID3.PLUGIN.MODULE_FEDERATION).use(rspack8.container.ModuleFederationPlugin, [options]);
        if (options.name) {
          chain.plugin("mf-patch-split-chunks").use(PatchSplitChunksPlugin, [options.name]);
        }
        const publicPath = chain.output.get("publicPath");
        if (publicPath === DEFAULT_ASSET_PREFIX4) {
  ...
  "src/plugins/rspackProfile.ts"() {
    ...
    pluginRspackProfile = () => ({
      ...
      async setup(api) {
        ...
        const traceFilePath = path13.join(profileDir, "trace.json");
  ...
  inspectCommand.description("inspect the Rspack and Rsbuild configs").option("--env <env>", "specify env mode", "development").option("--output <output>", "specify inspect content output path", "/").option("--verbose", "show full function definitions in output").action(async (options) => {
    try {
      ...
      await rsbuild?.inspectConfig({
        ...
        outputPath: join12(rsbuild.context.distPath, options.output),
        writeToDisk: true
      });
    } catch (err) {
